...
- **T-1/T-1M/T-2 implemented (shadow + transition monitor + arm-gate, read-only)** · `trendManualPaperArmGate` (operator arm paper-only, armed ≠ executed) · **T-3 paper simulated execution = DESIGN ONLY** (`docs/TREND_STRATEGY_T3_PAPER_EXECUTION_PLAN.md`): `trendPaperOrder` (PAPER_SIMULATED) + `trend_paper_journal.jsonl` แยก + conservative fill (SL-before-TP) + risk limits + trend evidence metrics · **แยกจาก grid เด็ดขาด** (trendClosedTrades/trendExpectancy ≠ grid closedCycles/expectancy) · no live · no BingX execution API · no old grid BUY→trend SELL · T-4 Edge Review (expectancy/drawdown) ≠ live · live ต้อง approval แยก + M-0B BLOCKED
