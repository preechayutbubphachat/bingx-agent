...
- **T-1/T-1M implemented (shadow + transition monitor, read-only)** · **T-2 manual paper arm = DESIGN ONLY** (`docs/TREND_STRATEGY_T2_MANUAL_PAPER_ARM_PLAN.md`): `trendManualPaperArmGate` (operator arm paper-only เมื่อ AWAITING_CONFIRMATION+risk PASS+confirm+regime match+expiry) · **armed ≠ executed** (execution = T-3) · arm ไม่สร้าง order/fill/evidence · paper/live activation false · แยกจาก Phase 2-B + EXCHANGE_MANUAL_APPROVAL
