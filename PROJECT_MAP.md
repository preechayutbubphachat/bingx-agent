...
- **T-3G Operator Dry Run Console (BACKEND-ONLY, security-blocked UI):** route `dashboard/app/api/internal/trend-paper-dry-run/route.ts` (auth-gated, 5 actions: baseline_check/create_session/verify_session/one_shot_run/cleanup) · helper `lib/trend/trendPaperDryRun.ts` (forced maxEntries=1, expiry≤30m, flags false) · UI card `TrendPaperDryRunConsoleCard.tsx` = **read-only, ปุ่ม disabled** (ไม่มี token/fetch ใน browser) → classify `T3G_BACKEND_ONLY_SECURITY_BLOCK` · ใช้ T-3F shell pack ไปก่อนจนกว่าจะมี admin-auth layer
- **T-3H Evidence Runner (DESIGN ONLY):** `docs/TREND_STRATEGY_T3H_EVIDENCE_RUNNER_DESIGN.md` — เก็บ closed trend trades ≥30/≥100 เพื่อประเมิน expectancy · architecture แนะนำ = env-gated internal evidence-cycle route + cron trigger (token จาก env ไม่ browser) · gate ชั้นสอง `TREND_PAPER_EVIDENCE_RUNNER_ENABLED` + daily caps/cooldown/consec-loss · one-entry rolling sessions · **gap audit: exit lifecycle มี logic แต่ยัง unverified + journal ขาด indicator/holdTime/MAE/MFE** → ต้องทำ T-3H-2 ก่อน · classify `T3H_DESIGN_READY`
- **T-3H-4 Evidence Runner (DESIGN ONLY):** `docs/TREND_STRATEGY_T3H4_EVIDENCE_RUNNER_DESIGN.md` — server เก็บ evidence อัตโนมัติ ภายใต้ env double-gate (`TREND_PAPER_SIMULATION_ENABLED` + `TREND_PAPER_EVIDENCE_RUNNER_ENABLED`) + budget caps (1 open/3 entries-day/3R loss/3 consec/60m cooldown/BTC-USDT) · state file `trend_paper_evidence_state.json` (path-locked) · runner = orchestration layer reuse T-3G/T-3H-2 (engine/bridge/append-consume/metrics ไม่แตะ) · 1 entry/1H bar · fail-closed · **cron ไม่ติดตั้งจนอนุมัติ** · classify `T3H4_DESIGN_READY`
- **T-3H-4-a/b/c Evidence Runner (IMPLEMENTED, paper-only, cron NOT installed):** core `lib/trend/trendPaperEvidenceState.ts`+`trendPaperEvidenceRunner.ts` (pure, injected deps, state machine A→H, fail-closed) · route `/api/internal/trend-paper-evidence-cycle` (auth-gated GET status + POST run_once) · read-only card `TrendPaperEvidenceRunnerCard.tsx` (ปุ่ม disabled) · scheduler `dashboard/scripts/trend_paper_evidence_cycle.sh` (single-shot, token จาก env, fail-closed exit codes 2-8, **cron proposal เป็น comment ไม่ติดตั้ง**) · ต้องเปิด 2 env gates เอง (`TREND_PAPER_SIMULATION_ENABLED`+`TREND_PAPER_EVIDENCE_RUNNER_ENABLED`) จึงเก็บ evidence จริง · classify `T3H4C_SCHEDULER_SCRIPT_READY`
- **T-3H-6-a Rejection Decision Log (IMPLEMENTED observability-only):** `dashboard/lib/trend/trendEvidenceDecisionLog.ts` writes append-only JSONL at `dashboard/tmp/trend-paper/trend_paper_evidence_decisions.jsonl` after evidence state write succeeds in POST `run_once`; GET remains read-only; `trendPaperEvidenceRunner.ts` decision logic never imports/reads the log; summary is exposed read-only through `paperLoopDiagnostics.trendEvidenceDecisionSummary` and Agent HQ `RejectionAnalysisCard`; design doc `docs/T-3H-6a_rejection_frequency_design.md`; no threshold/gate/order/live change.
- **T-3H-6-c MTF OB/FVG Entry Refinement Shadow (IMPLEMENTED read-only):** pure helper `dashboard/lib/trend/mtfObFvgRefinementShadow.ts` + Agent HQ `MtfObFvgShadowCard` estimate whether OB/FVG-style entry refinement could improve RR geometry; exact OB/FVG zones are not inferred when missing, fallback is labeled heuristic; doc `docs/T-3H-6c_mtf_ob_fvg_refinement_shadow.md`; no threshold, entry, runner, order, live, or adaptive RR activation.
- **T-3H-6-c1 MTF OB/FVG Shadow Snapshot Logging (IMPLEMENTED observability-only):** optional `rrSnapshot` + `smcMtfShadowSnapshot` fields are appended to `trend_paper_evidence_decisions.jsonl` only after evidence state write succeeds; read-only summary is exposed through `trendEvidenceDecisionSummary.mtfObFvgShadowSummary` and Agent HQ `MtfObFvgShadowCard`; malformed/old records are skipped safely; no threshold, entry, runner decision, adaptive RR, OB/FVG execution, order, or live impact.
- **T-3H-6-c2 MTF OB/FVG Shadow Review (IMPLEMENTED review-only):** pure helper `dashboard/lib/trend/mtfObFvgShadowReview.ts` reviews `mtfObFvgShadowSummary` sample tier, evidence grade, exact-zone readiness, and next step in Agent HQ; current exact-zone readiness is heuristic unless structured exact OB/FVG coordinates exist; no threshold, entry, runner decision, adaptive RR, OB/FVG execution, order, or live impact.
...
- **T-3E (implemented):** `trendPaperArmSessionWriter` persist `usedEntries` หลัง `TREND_PAPER_ENTRY` journal append สำเร็จเท่านั้น (append-first → consume-second) · consume path lock = `dashboard/tmp/trend-paper/trend_paper_arm_session.json` · ถ้า append fail = ไม่ consume · ถ้า consume fail หลัง append = warning/operator inspect session manually · cron ยัง disabled
