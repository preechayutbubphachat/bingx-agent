...
function Widget({
  ...
  return (
    <section className="min-h-[180px] rounded-lg border border-[#3a2c21]/10 bg-[#fffaf1] p-3 shadow-sm">
      <h2 className="mb-2 text-xs font-black uppercase tracking-wide text-[#5b4432]">{title}</h2>
      {children}
    </section>
  );
}

function MiniProgress({ label, status }: { label: string; status: "PASS" | "DATA_GAP" | "BLOCKED" | "PENDING" }) {
  const cls =
    status === "PASS"
  ...
  return (
    <div className="flex items-center justify-between gap-2 border-b border-[#3a2c21]/10 py-1.5 text-xs last:border-0">
      <span className="text-[#6d5745]">{label}</span>
      <span className={`rounded-full px-2 py-0.5 text-[10px] font-black ${cls}`}>{status}</span>
    </div>
  );
}
...
export default function BottomWidgetDock({
  ...
  const closedCycleStatus = vm.paper.closedCycles > 0 ? "PASS" : "DATA_GAP";
  const sampleStatus = vm.paper.sampleStatus === "SUFFICIENT" ? "PASS" : "DATA_GAP";
  ...
  return (
    <div className="grid grid-cols-1 gap-3 xl:grid-cols-5">
      ...
      <Widget title="Evidence Progress">
        <MiniProgress label={`Paper fills: ${vm.paper.totalOrderFilled}`} status={vm.paper.totalOrderFilled > 0 ? "PASS" : "PENDING"} />
        <MiniProgress label={`Closed cycles: ${vm.paper.closedCycles}`} status={closedCycleStatus} />
        <MiniProgress label="Sample size" status={sampleStatus} />
        <MiniProgress label={`Edge status: ${vm.paper.edgeStatus}`} status={vm.paper.edgeStatus === "DATA_GAP" ? "DATA_GAP" : "PENDING"} />
